jQuery(document).ready( $ => {
  $(function () {
    $(document).scroll(function () {
      var $head = $("header");
      $head.toggleClass('scrolled', $(this).scrollTop() > ($head.height() * 10 ));
    });
  });
});
...
function toggleDashiconClass() {
  ...
  if ( button.contains('dashicons-controls-play') ) {
    
    button.remove('dashicons-controls-play');
    
    button.add('dashicons-controls-pause');

  } else {
    button.remove('dashicons-controls-pause');
    
    button.add('dashicons-controls-play');
  }
